...
if __name__ == "__main__":
	...
	for j in range(len(dataset)):
		mi.append(mutual_information(algorithm.featurizer,eval_loaders[t], eval_loaders[j]))
	...
	for j in range(len(in_splits)):
		if j != t:
			...
			sorted_data = [[] for _ in range(algorithm.num_classes)]
			for i, lbl in enumerate(y):
				sorted_data[lbl].append(p[j])
			sorted_data = [torch.stack(class_data).cpu() for class_data in sorted_data]

			for c in range(self.num_classes):
				u,s,vt = torch.svd(sorted_data[j])
				components.setdefault(c,[]).append(vt.t()[:hparams['n_comp']])
				singular_values.setdefault(c,[]).append(1/mi[j]*s[:hparams['n_comp']])

	for c in range(self.num_classes):
		algorithm.components[c] = torch.cat(components[c])
		algorithm.singular_values[c] = torch.cat(singular_values[c])

	evals = zip(eval_loader_names[t], eval_loaders[t], eval_weights[t])
	results = {}
